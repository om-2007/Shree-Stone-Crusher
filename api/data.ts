import { pool, decrypt, safeInitDb } from './_lib/db.js';
...
export default async function handler(req: any, res: any) {
  try {
    ...
    // Attempt to fetch from khata_payments, fallback to khata_logs if necessary
    let khata_payments: any[] = [];
    try {
      const result = await pool.query("SELECT * FROM khata_payments");
      khata_payments = result.rows;
    } catch (err) {
